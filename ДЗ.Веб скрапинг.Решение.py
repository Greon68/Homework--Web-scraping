...
# (https://spb.hh.ru/) самыми первыми :)
#
# 1) Необходимо парсить страницу со свежими вакансиями с поиском по "Python" и городами
# "Москва" и "Санкт-Петербург". Эти параметры задаются по ссылке -
...
# Вытащили содержимое главной страницы :
main_html_data = response.text
# Распарсили  главную страницу
...
for vacancy in vacancys_tags:
    ...
    # Данные попадают в выходной список , только если содержимое объявлений
    # отвечает требуемым критериям соответсвия по городу и требуемым навыкам :
    if 'Москва' in toun and len(skill_list) > 0 or 'Санкт-Петербург' in toun and len(skill_list) > 0:
        parsed_data.append(
        {
         "title": title,
         "link": link,
